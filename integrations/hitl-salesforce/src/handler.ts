...
export const handler: IntegrationProps['handler'] = async ({ req, ctx,client, logger }) => {
  ...
  if(trigger.type == 'TRANSPORT_START' || (trigger.type == 'DATA' && (!trigger.payload || (typeof trigger.payload == 'string' && !trigger.payload.length)))) {
    if(typeof trigger.payload == 'string' && !trigger.payload.length) {
      return
    }
    console.log(`Ignoring sf event of type: ${trigger.type} with definition ${JSON.stringify(trigger, null, 2)}`)
    return
  }
  ...
  if(['DATA', 'TRANSPORT_END', 'TRANSPORT_RESTORED', 'ERROR'].includes(trigger.type)) {
    ...
    switch (trigger.type) {
      case 'DATA':
       ...
       } catch (e) { return /* Ignore non json data */ }

        console.log('Got conversation for data: ', { conversation })
        switch (messagingTrigger.event) {
          case 'CONVERSATION_MESSAGE': await executeOnConversationMessage({ messagingTrigger, conversation, client, logger }); break;
          case 'CONVERSATION_PARTICIPANT_CHANGED': await executeOnParticipantChanged({ messagingTrigger, ctx, conversation, client, logger }); break;
          case 'CONVERSATION_CLOSE_CONVERSATION': await executeOnConversationClose({ messagingTrigger, ctx, conversation, client, logger }); break;
          default: console.log('Got unhandled event: ' + trigger.payload.event)
        }
        return
      case 'ERROR':
        // If you start the polling session with debug enabled
        logger.forBot().debug(`Got a debug error from the transport session: ` + JSON.stringify({ trigger, response: trigger.payload?.response }, null, 2))
        return
      case 'TRANSPORT_END':
      ...
      case 'TRANSPORT_RESTORED':
        ...
        if(isConversationClosed(conversation)) {
          console.log('Restored transport from a conversation that is already closed, ending transport')
